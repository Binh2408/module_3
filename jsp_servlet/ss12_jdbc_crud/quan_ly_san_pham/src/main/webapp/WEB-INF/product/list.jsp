    ...
    <div class="d-flex justify-content-center align-items-center gap-2">
      ...
      <select name="searchCategory" class="form-select w-auto me-2">
        ...
        <c:forEach var="category" items="${categoryList}">
          <option value="${category.tenLoai}"
                  <c:if test="${param.searchCategory == category.tenLoai}">selected</c:if>>
              ${category.tenLoai}
          </option>
        </c:forEach>
      </select>
    ...
    <c:forEach var="item" items="${productDTOList}" varStatus="idx">
      <tr>
        ...
        <td>
          ...
          <button class="btn btn-primary" onclick="deleteInfo('${item.productID}', '${item.productName}')">Xóa</button>
        </td>
      </tr>
    </c:forEach>
  </table>
